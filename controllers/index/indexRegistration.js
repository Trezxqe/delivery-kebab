...
const indexRegistration = (req, res) => {
  ...
  .then(hash => {
    ...
    .then(data => {
      ...
      res.json({ message: `Пользователь ${username} успешно зарегистрировался`})
    })
    .catch((err) => res.json({ message : `Ошибка. Пользователь с таким именем (${username}) уже зарегистрирован`}))
  })
};
